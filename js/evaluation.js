const evaluateBoard = (board, maximizingPlayer) => {
    ...
    const analyze5 = (r, c, dr, dc, boundaryCheck) => {
        ...
        if ((count[BLACK] < 3 || count[WHITE] != 0) && (count[WHITE] < 3 || count[BLACK] != 0)) return [r, c];

        let color = count[1] ? BLACK : WHITE;
        let opponent = color == BLACK ? WHITE : BLACK;

        if (boundaryCheck && board[r][c] == 0 && board[r + 5 * dr][c + 5 * dc] == 0) {
        ...
        if (!boundaryCheck || board[r + 5 * dr][c + 5 * dc] == opponent) {
            threats[3][color - 1]++;
        }
    ...
    let maxPlayer = maximizingPlayer ? ai : human;
    let minPlayer = maximizingPlayer ? human : ai;

    if (threats[0][maxPlayer - 1] > 0 || threats[1][maxPlayer - 1] > 0) {
        scores[maxPlayer - 1] += 10000;
    } else if (threats[0][minPlayer - 1] > 0 || threats[1][minPlayer - 1] > 1) {
        scores[minPlayer - 1] += 8000;
    } else if (threats[2][maxPlayer - 1] > 1) {
        scores[maxPlayer - 1] += 5000;
    } else if (threats[1][minPlayer - 1] + threats[2][minPlayer - 1] > 1) {
        scores[minPlayer - 1] += 3000;
    } else {
        if (threats[2][maxPlayer - 1] == 1) scores[maxPlayer - 1] += 500;
        if (threats[2][minPlayer - 1] == 1) scores[minPlayer - 1] += 300;
    }
